...
- Права(RequestRight) и исполняемые роли (ItRole) позволяют пользователю выполнять соответствующие операции в системе(например просматривать необходимые для пользователя отчеты).
...
- Необходимо провести рефакторинг кода библиотеки коннектора без потери функциональности.

## Замечания
- Конфигурация коннектора происходит через метод StartUp 
